<?php
class cross_finder extends MainFuncClass {
	protected $templateName = '04/safety_survey/cross_finder/cross_finder.tpl';
	...
	public function execute(){
		...
		if($confirm_button != ''){
			...
			if ( $confirm_button == '' ){
				...
				$this->setTemplateName( '04/safety_survey/cross_finder/cross_finder.tpl' );
				return;
			}
		...
		if($send_button != ''){
			// 災害用伝言板サービスが提供されているか調べる
			if(is_dengonban() == FALSE){
				$this->smarty->assign('message','エラー：現在、災害用伝言板サービスは提供されていません。<br />伝言板検索を中止しました。');
				$this->setTemplateName( '04/safety_survey/cross_finder/cross_finder_complete.tpl' );
				return;
			}
			...
			$cnt = 0;
			foreach($phone_num as $key1 => $val1){
				if(cross_finder($val1) == TRUE){
					$cfDao->insert($mem_id[$key1], $survey_id);
					if($result){   // 登録成功
						$cnt++;
					}
				}
		...
		if($send_button != ''){
			...
			$this->setTemplateName( '04/safety_survey/cross_finder/cross_finder_confirm.tpl' );
		}else{
			$this->setTemplateName( '04/safety_survey/cross_finder/cross_finder.tpl' );
		}
	}

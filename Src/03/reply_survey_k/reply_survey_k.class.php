<?php
    class reply_survey_k extends MainFuncClass {
        protected $templateName = "03/reply_survey_k/reply_survey_k.tpl";
        ...
        public function execute() {
            ...
            // URLまたはPOSTのキーを調べる
            if(isset($_GET['key']) || isset($_POST['key'])){   // キーを取得してデータベースと照合する
                if(isset($_GET['key'])){
                    $key = htmlspecialchars($_GET['key']);
                }else{
                    $key = htmlspecialchars($_POST['key']);
                }
				$url_array = $url_dao->select_key($key);
                if(count($url_array)==1){
                    if($url_array[0]['isable'] == 1 && $url_array[0]['type'] == 1){
                        $reply_id = $url_array[0]['member_id'];
                    }else{
                        $error_message = '安否回答者対象外あるいはすでに回答済みのため、安否回答はできません。';
                        $this->smarty->assign('error_message', $error_message);
                        $this->setTemplateName("03/reply_survey_k/reply_survey_error_k.tpl");
                        return;
                    }
                }
            }else{   // キーが取得できない
                $error_message = 'エラー：認証キーが取得できませんでした。';
                $this->smarty->assign('error_message', $error_message);
                $this->setTemplateName("03/reply_survey_k/reply_survey_error_k.tpl");
                return;
            }
			...
			$status_array = $status_dao->select_byMember($reply_id);
			...
			foreach($survey_array as $data){
                ...
                if(in_array($reply_id, $tmp2) === TRUE){
            ...
            if($send_button != ''){
                ...
                $result = $status_dao->insert($survey_id,$reply_id,$safety_status,$location,$attend_school,$comment);
                
                // キーのフラグを0にする
                $result2 = $url_dao->update($reply_id,$survey_id,0);
                ...
                $this->setTemplateName("03/reply_survey_k/reply_survey_complete_k.tpl");
            }
            ...
            if($send_button != ''){
                ...
                $this->setTemplateName("03/reply_survey_k/reply_survey_confirm_k.tpl");
            }else{
                $this->setTemplateName("03/reply_survey_k/reply_survey_k.tpl");
            }
        }
    }
?>